# Лабораторная работа 3 — модель песчаной кучи и BMP

Реализуется абелева песчаная куча: стартовое распределение песчинок читается из файла, далее модель итеративно обваливается и свои состояния сохраняет в BMP‑картинки.


Так же в качестве доп таски я сделал, чтобы входной файл мог быть BMP (количество песчинок в точке равно сумме значений цвета в RGB)
...
```bash
cd labwork3-SandPile
cmake -S . -B build
cmake --build build

./build/labwork3-maksimbelov1 \
  --input=examples/start.tsv \
  --output=out \
  --max-iter=50000 \
  --freq=5
```

## Пример получившейся картинки (начальное состояние 100к песчинок в координате (0 : 0))

![100к точек в координате (0: 0)](img/image.png)
...
Реализовать [модель песчаной кучи](https://en.wikipedia.org/wiki/Abelian_sandpile_model), которая позволяет сохранять свои состояния в картинку в [формате BMP](https://en.wikipedia.org/wiki/BMP_file_format).
